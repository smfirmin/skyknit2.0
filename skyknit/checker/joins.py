...
from skyknit.topology.types import ArithmeticImplication, Join
from skyknit.utilities.conversion import stitch_count_to_physical
from skyknit.utilities.types import Gauge


def _parse_ratio(ratio: str) -> float:
    """Parse a pickup ratio string into a float.

    Accepts both colon-separated form (``"2:3"`` → 2/3) and plain decimal
    strings (``"0.667"`` → 0.667).
    """
    if ":" in ratio:
        num, denom = ratio.split(":", 1)
...
def validate_join(
    ...
    match arith:
        ...
        case ArithmeticImplication.RATIO:
            pickup_ratio = _parse_ratio(str(join.parameters["pickup_ratio"]))
