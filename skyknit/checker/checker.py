...
from skyknit.checker.joins import validate_join
from skyknit.checker.simulate import CheckerError, extract_edge_counts, simulate_component
...
def check_all(
    ...
    for component_spec in manifest.components:
        ...
        edge_counts = extract_edge_counts(ir, component_spec, manifest.joins)
    ...
    for join in manifest.joins:
        # PICKUP joins from a SELVEDGE edge require row-count arithmetic:
        #   sleeve_sts = body_rows × pickup_ratio
        # But the pipeline derives sleeve stitch count from upper_arm_circumference
        # × ease (measurement-driven), not from body_rows × pickup_ratio
        # (topology-driven).  These two values are not generally equal, so RATIO
        # validation would always fail.  Skip SELVEDGE-source PICKUP joins in v1.
        if _edge_type_for_ref(join.edge_a_ref, manifest) == EdgeType.SELVEDGE:
            continue
...
def _edge_type_for_ref(edge_ref: str, manifest: ShapeManifest) -> EdgeType | None:
    """Return the EdgeType for a 'component.edge' reference, or None if not found."""
    comp_name, edge_name = edge_ref.split(".", 1)
